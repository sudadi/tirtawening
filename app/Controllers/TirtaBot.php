...
class TirtaBot extends BaseController
{
  ...
  public function bothook()
  {
    ...
    $botconfig = new \Config\Telebot();

    try {
      // Create Telegram API object
      $telegram = new Telegram($botconfig->api_key, $botconfig->bot_username);

      // Enable admin users
      $telegram->enableAdmins($botconfig->admins);

      // Add commands paths containing your custom commands
      $telegram->addCommandsPaths($botconfig->commands['paths']);
      ...
      $telegram->setDownloadPath($botconfig->paths['download']);
      $telegram->setUploadPath($botconfig->paths['upload']);
      ...
      $telegram->enableLimiter($botconfig->limiter);
